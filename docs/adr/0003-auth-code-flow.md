...
You serves its own login UI (LiveView). Apps redirect users to You and receive a single-use authorization code back via redirect callback. Apps exchange that code for a JWT by calling `You.IAM.Server` over Erlang distribution — no JWT ever appears in a URL.
...
- Authorization codes reuse the existing `users_tokens` table with `context: "oauth_code"` — no migration needed.
