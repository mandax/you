...
| `TRUSTED_PROXY_HOPS` | No | `0` | Number of reverse proxies in front of this container that append to (not replace) `X-Forwarded-For`. The `docker run` example above publishes the port directly with no proxy in front, so `0` (unset) is correct as shown — the login rate limits then key on `conn.remote_ip`, the container's actual TCP peer. Put a reverse proxy in front (as [deploy.md](deploy.md#https) requires for HTTPS) and this has to change to match, or every caller behind that proxy shares its one bucket instead of getting their own. Do not guess the count — see [deploy.md](deploy.md#determining-your-hop-count), since a proxy in front of a proxy (a CDN in front of your own nginx or Caddy, say) is more than one hop and a tunnel client is often not an extra hop at all. Capped at 20 — an oversized value fails the boot naming the cap. Environment-only, same reasoning as `WEBAUTHN_RP_ID` |
| `TRUSTED_PROXY_HOPS_DEBUG` | No | (none) | Temporary diagnostic: any non-empty value logs the raw `X-Forwarded-For` entries and resolved client IP for every rate-limited request, so you can confirm `TRUSTED_PROXY_HOPS` against a real request. See [deploy.md](deploy.md#determining-your-hop-count) |
